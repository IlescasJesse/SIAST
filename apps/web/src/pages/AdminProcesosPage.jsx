...
const EMPTY_FORM = {
  subcategoria: "EQUIPOS_DISPOSITIVOS",
  subTipo: "",
  tipoFlujo: "DIRECTO",
  nombre: "",
  descripcion: "",
      ...
      <Box sx={{ flex: 1, display: "flex", flexDirection: "column", gap: 1 }}>
        <Box sx={{ display: "flex", gap: 1 }}>
          ...
          <FormControl size="small" sx={{ minWidth: 200 }}>
            ...
            <Select
              value={paso.rolRequerido}
              label="Rol requerido"
              onChange={(e) => onChange(paso.tempId, "rolRequerido", e.target.value)}
            >
        ...
        <Box sx={{ display: "flex", gap: 1, alignItems: "center" }}>
          <TextField
            ...
            fullWidth
            onChange={(e) => onChange(paso.tempId, "descripcion", e.target.value)}
          />
          <FormControlLabel
            control={
              <Switch
                size="small"
                checked={paso.registraUnidades}
                onChange={(e) => onChange(paso.tempId, "registraUnidades", e.target.checked)}
              />
            }
        ...
        {paso.registraUnidades && (
          <TextField
            size="small"
            label='Etiqueta de unidades (ej: "PCs atendidas")'
            value={paso.labelUnidades}
            onChange={(e) => onChange(paso.tempId, "labelUnidades", e.target.value)}
          />
        )}
      </Box>
...
export const AdminProcesosPage = () => {
  ...
  const guardar = async () => {
    ...
    try {
      ...
      if (editId) {
        await updateProceso(editId, payload);
      } else {
  ...
  const handleToggle = async (p) => {
    try {
      await toggleProceso(p.id);
      cargar();
    } catch {
      setError("Error al cambiar estado del proceso");
    }
  };
                ...
                <TableRow key={p.id} sx={{ opacity: p.activo ? 1 : 0.5 }}>
                  ...
                  <TableCell>
                    <Typography variant="body2">
                      {LABEL_SUBCATEGORIA[p.subcategoria] ?? p.subcategoria}
                    </Typography>
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell>
                    <Chip
                      label={p.tipoFlujo}
                      size="small"
                      color={CHIP_FLUJO[p.tipoFlujo] ?? "default"}
                    />
                  </TableCell>
                  <TableCell>
        ...
        <DialogContent dividers>
          <Box sx={{ display: "flex", flexDirection: "column", gap: 2, pt: 1 }}>
            ...
            <Box sx={{ display: "flex", gap: 2, alignItems: "center" }}>
              <FormControl size="small" sx={{ minWidth: 200 }}>
                ...
                <Select
                  ...
                  label="Tipo de flujo"
                  onChange={(e) => setForm((f) => ({ ...f, tipoFlujo: e.target.value }))}
                >
            ...
            <Box>
              ...
              >
                <SortableContext
                  items={form.pasos.map((p) => p.tempId)}
